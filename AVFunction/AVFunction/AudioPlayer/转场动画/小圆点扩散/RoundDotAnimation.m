...
-(void)presentViewController:(id <UIViewControllerContextTransitioning>)transitionContext{
    ...
    //选择X或者Y的最大值
    CGFloat x = MAX(tempViewController.presentNextController.x, containerView.width-tempViewController.presentNextController.x);
    ...
    masklay.path = endCycle.CGPath;
    //将masklayer赋给toVC。view；
    toViewController.view.layer.mask = masklay;
    //创建了路径动画
    CABasicAnimation *maskLayerAnimation = [CABasicAnimation animationWithKeyPath:@"path"];
    maskLayerAnimation.delegate = self;
    //动画加到layer上，必须为CGPath,再讲CGPath桥接为OC对象
    ...
    // 添加动画
    [masklay addAnimation:maskLayerAnimation forKey:@"path"];
}
