...
#import "LitterVideoController.h"
#import <AVFoundation/AVFoundation.h>
#import "mediaPlayController.h"
@interface LitterVideoController ()<AVCaptureFileOutputRecordingDelegate>
...
@implementation LitterVideoController
...
-(void)startCaptureWithSession{
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:self.videoPath]) {
        [[NSFileManager defaultManager] removeItemAtURL:[NSURL URLWithString:self.videoPath] error:nil];
    }
    NSError *error = nil;
...
-(BOOL)SetSessioninputs:(NSError *)error{
     ...
     */
    AVCaptureDevice *catureDevice = [AVCaptureDevice defaultDeviceWithMediaType:AVMediaTypeVideo];
    AVCaptureDeviceInput *videoInput = [AVCaptureDeviceInput deviceInputWithDevice:catureDevice error:&error];
    if (!videoInput) {
        return NO;
    }
    // 给捕获会话类添加输入捕获设备
...
-(void)startRecordSession{
    [self.captureMovieFileOutput startRecordingToOutputFileURL:({
        NSURL *url = [NSURL fileURLWithPath:[NSTemporaryDirectory() stringByAppendingString:@"XiaoShiPin.mov"]];
        NSLog(@"视频想缓存的地址：%@",url);
...
-(void)compressVideoWithFileUrl:(NSURL *)fileUrl{
    ...
    NSLog(@"视频压缩后的pressName:%@",exportSession.presetName);
